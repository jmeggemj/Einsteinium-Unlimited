...
Einsteinium Core is extensively tested on multiple operating systems using
...
Einsteinium Core should also work on most other Unix-like systems but is not
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/hashunlimited/einsteinium-unlimited/blob/master/doc/zmq.md) for
...
When Einsteinium Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `einsteinium-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`einsteinium-cli` has been updated to support this by parsing `name=value` arguments
...
The Alert System was disabled and deprecated in Einsteinium Core 0.10.4 and removed in 0.13.2. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Einsteinium Core. This was added in conjunction with
...
here, originally written for Bitcoin but can also be used for Einsteinium:
